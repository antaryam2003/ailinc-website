...
| 3   | Three (pinned reel) + reveal     | `sections/Three.tsx`        |
...
`public/platform/three/NN.webp` are the animated clips in the three reel
panels, same idea as the gallery clips: an animated WebP over a static first
frame, mounted only while that panel is the active one.

| Panel | Source GIF | Shipped | Duration |
| ----- | ---------- | ------- | -------- |
| 01 Personalize | 9.07 MB | 0.82 MB | 20.2s |
| 02 AI Teacher  | 8.19 MB | 0.77 MB | 18.6s |
| 03 Create      | 11.17 MB | 0.71 MB | 25.6s |

Three things differ from the recipe above.
...
libwebp merges identical frames into longer delays, so the frame count drops
but the duration does not.

**They are letterboxed to 864x540.** The recordings are about 1335x690 (ratio
~1.94); 864x540 is exactly 1.600, which is what the panel windows were built
around. Unpadded, each panel's window is its own height and the reel changes
shape as items swap. Cropping to 1.600 instead would cut ~72px off each side,
through the buttons at the right edge. The pad colour is sampled per clip from
that recording's own page background, so the bands do not read as bands.

**The animated `<img>` needs `loading="lazy"`.** Panel 01 is the active item at
rest, so its clip is in the first paint's markup and React emits a
...
**Clip 03 is redacted.** The catalogue it opens on shows a "STAFF PREVIEW"
badge on every course card — internal state, the same problem as gallery clip
04. The badges are found by colour rather than by fixed rectangle, because the
grid scrolls underneath them: the chip fill is `rgb(248,235,218)`, matched with
a small tolerance, grown into blobs, and filtered to chip proportions so warm
course thumbnails do not match. The chip's own text splits each one into a band
above and below, so those are merged before blurring. 207 chip instances across
51 frames. If clip 03 is re-recorded from a normal student account, drop the
redaction step.
...
| Pinned vertical reel       | Three                | 480vh track → continuous index MotionValue |
| Split-door reveal          | Three (same pin)     | panel halves part from the centre, the payoff emerges behind |
